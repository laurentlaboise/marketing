...
> **Status: implemented in this PR** (items 1-3, 5, 6, 8, 10 fully; item 4 as authoritative disk+DB+GitHub checks without the UNIQUE index — applying an index blindly could fail on pre-existing duplicate rows, so it needs a data-dedup pass first; item 7 as a "Used by" panel on the detail page plus warnings on rename, rather than blocking confirmations; item 9 — machine API hardening — remains open). Rename now publishes to the CDN before touching anything, refuses to clobber existing names, and warns about references; optimize/bulk never delete the old file until the new one is confirmed published; Replace re-encodes in the current format so the URL genuinely stays the same; animated images convert to animated WebP or are refused/skipped instead of silently flattened; bulk-optimize reports CDN failures and skips; archived images have a filter and Restore action.
...
> **Status: partially implemented in this PR.** Done: item 2 (`sitemap-images.xml` now committed by the nightly workflow); item 1's precondition (`/api/public/images/seo` lookups are exact-match on `cdn_url`/`file_path`/filename, with ambiguous results returning `image: null` + `ambiguous: true`); item 3 (article images are refreshed from the Image Library by id at read time in both public articles endpoints — this also heals renamed/converted URLs in articles); item 4's core (batch upload commits all files in ONE Git Data API commit, with per-file Contents API fallback — a 50-file upload is now 1 deploy, not 50); item 5 as tooltip honesty on "View on Web". Remaining: the build-time consumer that injects DB alt/title into generated static HTML + JSON-LD (a static-site build project), and a deliberate decision on skipping site rebuilds for image-only commits.
