// Workforce compensation defaults from the position briefs — shared by
// the CLI (scripts/setup-workforce.js) and the Payout Ledger's "Seed
// default rates" button, so going live never depends on shell access.
// Every figure is a starting point ("prices not set in stone") — adjust
// in Localization → Payout Ledger at any time. Idempotent: only fills in
// global rates that don't exist yet, never overwrites edits.
//
// Sources (kip):
...
async function seedDefaultRates() {
  ...
  for (const [workType, lang, rateType, amount, currency] of PAYOUT_RATE_DEFAULTS) {
    ...
    created += 1;
    log.push(`payout rate: ${workType} (${lang}) ${currency} ${amount.toLocaleString()} ${rateType}`);
  }
...
module.exports = { seedDefaultRates, PAYOUT_RATE_DEFAULTS, COMP_RATE_DEFAULTS };
