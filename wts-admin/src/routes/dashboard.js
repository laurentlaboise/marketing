...
router.get('/', async (req, res) => {
  try {
    ...
    const stats = await Promise.all([
      ...
      db.query("SELECT COUNT(*) FROM social_campaigns").catch(() => ({ rows: [{ count: 0 }] })),
      db.query("SELECT COUNT(*) FROM articles WHERE status = 'draft'").catch(() => ({ rows: [{ count: 0 }] })),
      db.query("SELECT COUNT(*) FROM social_posts WHERE status = 'scheduled'").catch(() => ({ rows: [{ count: 0 }] })),
    ...
    res.render('dashboard/index', {
      ...
      stats: {
        ...
        microsites: parseInt(stats[7].rows[0].count),
        campaigns: parseInt(stats[8].rows[0].count),
        drafts: parseInt(stats[9].rows[0].count),
        scheduledPosts: parseInt(stats[10].rows[0].count),
        activeSites: parseInt(stats[11].rows[0].count),
        unreadNotifications: parseInt(stats[12].rows[0].count)
      },
    ...
    res.render('dashboard/index', {
      title: 'Dashboard - WTS Admin',
      stats: { articles: 0, aiTools: 0, socialPosts: 0, products: 0, glossary: 0, seoTerms: 0, images: 0, microsites: 0, campaigns: 0, drafts: 0, scheduledPosts: 0, activeSites: 0, unreadNotifications: 0 },
      recentActivity: [],
      recentArticles: [],
      activeMicrosites: [],
      scheduledPosts: [],
